...
function processPartyMember(index, partyIcon, skillContainer, updateQuickPanels, pid) {
    ...
    if (updateQuickPanels) {
        ...
        if (!combatState.party[index].condition.ability_available_flag)
            quickPanel.classList.add("abilities-unavailable");
        else
            quickPanel.classList.remove("abilities-unavailable");
    ...
    for (var i = 0, l = 4; i < l; i++) {
        ...
        if (parsed) {
            ...
            else if (!combatState.party[parsed.characterNumber - 1].condition.ability_available_flag) {
                aState.textContent = "×";
            }
            else {
...
;
function parseServerPlayerState() {
    var trace = false;
    if (!serverSkillState)
    ...
    for (var frontRowCharacterNumber in lastPartyAbilityState) {
        ...
        // HACK
        characterState.condition.ability_available_flag =
            !(characterState.condition.ability_available_flag === false);
