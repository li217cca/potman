...
function onLoad() {
    ...
    document.querySelector("#joinRaid").addEventListener("click", doJoinRaid, false);
    ...
    chrome.tabs.query({
        ...
        chrome.runtime.sendMessage({ type: "getUserIdAndTabId", tabId: activatorTab.id }, function (uid, tabId) {
...
function useNormalItem(itemId, count) {
    useNormalItemCallback(itemId, count, function (result) {
        chrome.runtime.sendMessage({ type: "getUserIdAndTabId", tabId: activatorTab.id }, function (uid) {
