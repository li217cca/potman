...
function onAjaxComplete(bundle) {
    ...
    if (url.indexOf("/quest/create_quest") >= 0) {
        ...
        if (responseData) {
            ...
            parseServerPlayerState(responseData.player.param, responseData.ability);
        }
    ...
    switch (name) {
        case "battleFinish":
            ...
            if (combatState) {
                ...
                window.setTimeout(function () { checkForBattleResults(raidId, 1000, 0, url); }, 2350);
            }
            break;
        case "bossUpdate":
...
function onActionResult(data, uid, failed) {
    ...
    if (data.status && data.status.ability) {
        ...
        for (var i = 0, l = combatState.party.length; i < l; i++) {
            var j = combatState.characterIds.indexOf(combatState.party[i].pid) | 0;
            ps[j] = combatState.party[i];
        }
    ...
    for (var i = 0, l = data.scenario.length; i < l; i++) {
        ...
        if ((cmd.cmd === "super") && isRemoteConnected) {
            sendExternalMessage({
                type: "sendToRemote",
                data: {
                    type: "enemyUsedSuper",
