...
function processTurnChange(turnIndex) {
    ...
    resetCachedElements();
    updateSettings();
    chrome.runtime.sendMessage({ type: "updateRaidCode", raidCode: combatState.raidCode });
}
;
...
;
function scheduleGaugeTransplant(gauge, container) { // todo delete
    var gameContainer = getGameContainer();
    var effectiveZoom = getEffectiveZoom(gameContainer);
    var tryTransplant;
...
function refreshActionTimer() {
    ...
    if (!lastActionTimestamps) {
        chrome.runtime.sendMessage({ type: "getLastActionTimestamps", uid: currentBattleUid }, function (result) {
